...
      for (let i = 0; i < 40; i++) {
        const x = 50 + i * 7;
        const y = 200 + (Math.random() - 0.5) * 15; // Ajustado y a 200 para que se vea
        if (x < CANVAS_SIZE && y < CANVAS_SIZE) {
          newPoints.push({ x: Math.round(x), y: Math.round(y) });
        }
      }
...
      }
      
      // Círculo 2 (Ajustado a una posición visible: 80, 80)
      const cx2 = 80, cy2 = 80; 
      for (let angle = 0; angle < Math.PI * 2; angle += 0.25) {
        const x = cx2 + (radius / 2) * Math.cos(angle) + (Math.random() - 0.5) * 5; // Radio más pequeño
...
  const houghTransformLines = () => {
  ...
  const detected = [];
  // maxVal sigue siendo útil para el canvas, pero ya no para la detección
  ...
  for (let t = 1; t < thetaSteps - 1; t++) {
    for (let r = 1; r < rhoSteps - 1; r++) {
      ...
      // ---------- CAMBIO CLAVE 1 ----------
      // Criterio: superar umbral (se eliminó la condición 'maxVal * 0.3')
      if (accumulator[t][r] >= threshold) { 
        let isMax = true;
        // Supresión no máxima (Non-Maximum Suppression) en una ventana de 3x3
  ...
  // ---------- CAMBIO CLAVE 2 ----------
  // Ordenar (opcional) pero NO limitar
  detected.sort((a, b) => b.votes - a.votes);
  setDetectedShapes(detected.slice(0,100)); // Se eliminó .slice(0, 5)
};
...
      const angleStep = 10; // Grados 
      
      points.forEach(point => {
        for (let angle = 0; angle < 360; angle += angleStep) {
          const theta = (angle * Math.PI) / 180;
          // (xo, yo) = centro del círculo
...
          // Criterio: superar umbral (multiplicado por 2 debido a la resolución) Y tener al menos el 50% del voto máximo
...
            if (isMax) {
              detected.push({ 
                xo: x * HOUGH_RESOLUTION, // Mapear de vuelta a coordenadas de canvas
...
        
        // Las coordenadas X e Y en el espacio de Hough se invierten aquí para coincidir con la matriz [theta][rho]
        const x = (rhoIdx / houghSpace[0].length) * HOUGH_SIZE; // x ~ rho
        const y = (thetaIdx / houghSpace.length) * HOUGH_SIZE; // y ~ theta
        
        ctx.arc(x, y, 5, 0, Math.PI * 2);
      } else {
        // En el modo círculos, el acumulador es [xo][yo] con resolución reducida.
...
        <div style={{ 
          display: 'grid', 
          // MODIFICACIÓN: Simplificado el minmax a 320px para reducir el punto de quiebre
